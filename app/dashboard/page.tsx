...
export default function DashboardPage() {
  ...
  const [stockModal, setStockModal] = useState<{ show: boolean; productId: string; currentStock: number; newStock: string }>({ show: false, productId: "", currentStock: 0, newStock: "" });
  ...
  async function handleUpdateStock(e: React.FormEvent) {
    e.preventDefault();
    const parsedStock = parseInt(stockModal.newStock);
    ...
    const { error } = await supabase
      ...
      .eq("id", stockModal.productId);

    if (error) {
      alert("Error al actualizar stock: " + error.message);
    } else {
      setMsg("Stock actualizado correctamente.");
      setStockModal({ show: false, productId: "", currentStock: 0, newStock: "" });
      loadProductsOrdersAndPremiumRequests(); // Recargamos para que se vean los cambios
    }
  ...
  return (
    <main className="min-h-screen relative font-sans overflow-x-hidden bg-slate-900">
              ...
              {products.map((p) => {
                ...
                return (
                  <div key={p.id} className="product-shell">
                    <article className="group product-inner relative">
                      {p.image_url ? (
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        ...
                        {/* --- ETIQUETA INDICADOR DE STOCK ACTUAL --- */}
                      ...
                      {/* --- ALERTA ROJA GIGANTE DE FUERA DE STOCK --- */}
                      {p.stock !== undefined && p.stock <= 0 && (
                        <div className="mt-4 rounded-2xl border-2 border-red-500 bg-red-100 px-4 py-3 text-sm font-black text-red-700 flex items-center justify-center gap-2 shadow-md animate-pulse">
                          <span className="text-lg">❌</span> ¡TE QUEDASTE SIN STOCK!
                        </div>
                      )}
                      ...
                      <div className="mt-2">
                        <button
                          onClick={() => setStockModal({ show: true, productId: p.id, currentStock: p.stock || 0, newStock: String(p.stock || 0) })}
                          className={`flex h-12 w-full items-center justify-center gap-2 rounded-2xl px-4 text-center text-sm font-black text-white shadow-lg transition hover:brightness-110 active:scale-[0.98] ${p.stock && p.stock > 0 ? "bg-blue-600" : "bg-red-600"}`}
                        >
                          <span className="text-base leading-none">📦</span>
                          <span className="leading-none">{p.stock && p.stock > 0 ? "Modificar Stock" : "Recargar Stock"}</span>
                        </button>
                      </div>
      ...
      {/* --- MODAL PARA ACTUALIZAR STOCK (POP-UP) --- */}
      {stockModal.show && (
        <div 
          className="fixed inset-0 z-[150] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
          onClick={() => setStockModal({show: false, productId: "", currentStock: 0, newStock: ""})}
        >
          <div 
          ...
          >
            ...
            <div className="mb-4 inline-flex items-center justify-center w-12 h-12 rounded-full bg-blue-100 text-blue-500 text-2xl">
              📦
            </div>
            <h3 className="text-xl font-black text-slate-800 mb-2">Actualizar Stock</h3>
            <p className="text-slate-600 text-sm mb-4">
              Ingresa la cantidad actual que tienes disponible. Pon "0" si ya no tienes.
            </p>
            ...
            <form onSubmit={handleUpdateStock}>
              <input 
                ...
                onChange={e => setStockModal({...stockModal, newStock: e.target.value})}
                className="w-full rounded-2xl bg-slate-100 border-2 border-slate-200 px-4 py-3 text-lg font-black text-slate-800 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20"
                required
              />
              <button 
                type="submit"
                disabled={updatingStock}
                className="mt-6 w-full bg-[#2FA84F] hover:brightness-110 text-white font-black py-3 rounded-xl transition shadow-lg disabled:opacity-50"
              >
                {updatingStock ? "Guardando..." : "Guardar Stock"}
              </button>
            </form>
          </div>
        </div>
