...
export default function NewProductPage() {
  ...
  async function publicar(e: React.FormEvent) {
    ...
    // --- MAGIA: AVISO AUTOMÁTICO A LA IA PARA MODERACIÓN EN SEGUNDO PLANO ---
    if (insertedData && insertedData.id) {
      // Disparamos la petición sin el "await" para que no congele la página
      fetch("http://localhost:5000/api/moderar-listing", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        keepalive: true, // Esto evita que el navegador corte el aviso a Python al cambiar de pestaña
        body: JSON.stringify({ listing_id: insertedData.id }),
      }).catch((error) => console.log("Error al llamar a la moderación:", error));
    }
    // ------------------------------------------------------------------------

    // Redirigimos solos al dashboard después de 1.5 segundos exactos
    setTimeout(() => {
            ...
            <form onSubmit={publicar} className="mt-8 grid gap-3">
              ...
              >
                {openingImage
                  ? "⏳ Abriendo galería..."
                  : image
                  ? `✅ Imagen cargada: ${image.name} (Click para cambiar)`
                  : "🖼️ Seleccionar Imagen Click Aquí"}
              </button>
              ...
              <button
                type="submit"
                disabled={loading}
                className="rounded-2xl bg-[#F4C400] px-4 py-3 font-bold text-black transition hover:brightness-110 active:scale-[0.98] disabled:opacity-60 shadow-md flex justify-center items-center gap-2"
              >
                {loading ? (
                  <>
                    ...
                    Cargando... Su anuncio está siendo moderado
                  </>
                ) : (
                  "Publicar producto"
                )}
