...
export default function ProductPage() {
  ...
  // ESTADOS PARA LA COMPRA DIRECTA
  ...
  // FUNCIÓN PARA CREAR EL PEDIDO DESDE ACÁ MISMO
    ...
    <main className="min-h-screen relative font-sans overflow-x-hidden bg-slate-900">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 items-stretch">
              ...
              <div className={shellClass}>
                <div className={innerClass}>
                  {product.image_url ? (
                    <div 
                      className="group relative flex h-full min-h-[420px] w-full cursor-pointer items-center justify-center overflow-hidden rounded-2xl bg-gray-50 border-2 border-white"
                    ...
                    >
                      ...
                      />
                      {/* EFECTO LUPA AL PASAR EL MOUSE */}
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                        <div className="bg-white/90 text-slate-800 rounded-full px-5 py-2 font-black text-sm shadow-xl opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0">
                          🔍 Clic para ampliar
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="flex h-full min-h-[420px] w-full items-center justify-center rounded-2xl bg-gray-50 border-2 border-white text-sm text-slate-400 font-bold">
                      Sin imagen
                    </div>
                  )}
                </div>
              ...
              <div className={shellClass}>
                <div className={innerClass}>
                  ...
                  {/* BOTONES MEJORADOS (COMPRAR AHORA Y VOLVER) */}
                  <div className="mt-6 flex flex-col sm:flex-row gap-3">
                    <button
                      onClick={() => setBuyOpen(true)}
                      className="flex-1 rounded-2xl bg-[#F4C400] px-6 py-4 text-sm font-black text-black shadow-lg hover:brightness-110 active:scale-[0.98] transition uppercase tracking-wide flex items-center justify-center gap-2 border-2 border-transparent hover:border-yellow-200"
                    >
                      🛒 Comprar ahora
                    </button>
                    <Link
                      href="/"
                      className="rounded-2xl bg-white px-6 py-4 text-sm font-black text-slate-600 shadow-md border-2 border-slate-200 hover:bg-slate-50 hover:text-slate-800 active:scale-[0.98] transition flex items-center justify-center"
                    >
                      ← Volver al inicio
                    </Link>
                  </div>
      ...
      {isZoomed && product?.image_url && (
        ...
        >
          ...
          />
          <button 
            className="absolute top-6 right-8 text-white text-4xl font-black hover:text-red-500 transition-colors"
            onClick={(e) => {
              e.stopPropagation();
              setIsZoomed(false);
            }}
      ...
      {/* --- MODAL DE COMPRA (IGUAL AL DE LA PÁGINA PRINCIPAL) --- */}
      {buyOpen && product && (
          ...
          <div className="login-shell w-full max-w-md rounded-[2rem] p-4">
            <div className="login-inner rounded-[1.7rem] p-6 relative">
              ...
              <button
                ...
                className="absolute top-4 right-5 text-slate-400 hover:text-red-500 text-2xl font-bold"
              >
                ✕
              </button>
